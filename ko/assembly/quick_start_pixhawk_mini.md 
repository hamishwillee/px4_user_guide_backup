...
This quick start guide shows how power the [Pixhawk Mini](pixhawk_mini.md) and connect its most important peripherals (including output connections for copter, plane, and VTOL vehicles).
...
![Pixhawk Mini recommended orientation](../../images/pixhawk_mini_mounting_arrow.png) ![Mounting foam](../../images/3dr_anti_vibration_mounting_foam.png)
...
> **Note** The *Quad Power Distribution Board* includes a power module (PM) that is suitable for batteries <= 4S. The [3DR 10S Power Module](https://store.3dr.com/products/10s-power-module) is recommended if you need more power.
...
- PPM and S.BUS receivers must connect to the **RCIN** port. ![Pixhawk Mini - Radio port for PPM receivers](../../images/pixhawk_mini_port_rcin.png)
- PWM receivers (with individual cables for each channel) must connect to the RCIN channel *via* a PPM encoder [like this one](http://www.getfpv.com/radios/radio-accessories/holybro-ppm-encoder-module.html).
- Spektrum and DSM receivers must connect to the **SPKT/DSM** input. ![Pixhawk Mini - Radio port for Spektrum receivers](../../images/pixhawk_mini_port_spkt_dsm.png)
